...
const Tutorial = () => {
  return (
    ...
    >
      ...
      <div className="absolute -left-[400px] top-1/2 transform -translate-y-1/2 w-[1000px] h-[1000px] bg-secondary rounded-full blur-3xl" />
      ...
      <div className="max-w-[1300px] w-full flex flex-col justify-start items-center gap-y-16">
        ...
        <div className="relative flex flex-col items-center gap-y-6">
          ...
          {/* Tutorial image */}
          <img 
            src="/tutorial_display.png" 
            className='rounded-lg brightness-[95%] px-4 max-md:px-0 max-lg:w-full max-md:mb-6'
          />
          ...
          >
            {steps.map((step, index) => (
              <div 
                key={index} 
                className="p-4 flex flex-col gap-y-4 max-w-[400px] w-full h-[150px] max-[1022px]:max-w-none max-[1022px]:h-auto bg-[#FAFAFA] shadow-md rounded-lg"
              >
                {/* Step icon */}
                <div className="bg-header flex justify-center items-center w-[30px] h-[30px] rounded-full text-white/80">
                  {step.icon}
                </div>
                <div>
