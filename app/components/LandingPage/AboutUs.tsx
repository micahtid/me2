      ...
      <div className="relative default-container space-y-12">
        ...
        <div className="relative space-y-5 max-w-2xl mx-auto text-center">
          ...
          <p className="dynamic-text text-black/70">
            Let our custom algorithm match you with people who have high
            compatibility ratings. Send a request, start chatting for 48 hours
            before the chat room closes. It&apos;s that simple. Find someone
            like you.
          </p>
        </div>
              ...
              {about[activeIndex].points.map((point, index) => (
                <motion.div
                  ...
                  className="flex items-center space-x-4 p-6 rounded-xl bg-white shadow-sm 
                                    group transition-color duration-300 hover:bg-primary/50"
                >
                  <div
                    className="flex-shrink-0 p-2 bg-primary/20 rounded-lg
                                    transition-color duration-300 group-hover:bg-white/50"
                  >
                    <FaCheck className="w-4 h-4 text-header" />
                  </div>
                  <p className="text-black/70">{point}</p>
                </motion.div>
              ))}
            </motion.div>
          </AnimatePresence>
