...
const RoomsDisplay = () => {
  ...
  const sortedRooms = rooms?.slice().sort((roomA, roomB) => {
    // Move user's active room to the top of the list (!)
    const userInRoomA =
      roomA.roomId === user?.uid || roomA.users.includes(user?.uid);
    const userInRoomB =
      roomB.roomId === user?.uid || roomB.users.includes(user?.uid);

    return userInRoomB - userInRoomA; // Place user's room before other rooms (!)
  });

  const userInAnyRoom = sortedRooms?.some(
    (room) => room.roomId === user?.uid || room.users.includes(user?.uid),
  ); // Status if user is currently in any other rooms

  return (
    <div className="flex flex-col gap-y-3 px-8 justify-start items-start w-full">
        ...
        >
          <div className="flex justify-between w-full items-start gap-x-4 px-8 pt-6">
            <div className="flex flex-col">
              <div className="flex flex-col gap-x-3 mb-1">
          ...
          <div className="flex flex-wrap gap-2 px-8 pb-6">
            {room.tags.map((tag: string, index: number) => (
              <span
                ...
                "
              >
                {roomTags.find((roomTag) => roomTag.value === tag)?.label ||
                  tag}
              </span>
            ))}
          </div>
